...
class APIManager
{
    ...
    public function getPermits ($county, $token, $date, $interestArea) {
        ...
        if ($interestArea == 'nm' || $interestArea == 'admin') {
            $url = "https://di-api.drillinginfo.com/v2/direct-access/permits?approveddate=ge(".$date.")&countyparish=".$county."&drilltype=in(H)&pagesize=2000";
        } else {
            $url = "https://di-api.drillinginfo.com/v2/direct-access/permits?submitteddate=ge(".$date.")&countyparish=".$county."&drilltype=in(H)&pagesize=2000";
        }
            $curl = curl_init();
